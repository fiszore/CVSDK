...
{
    public class GetShovedCV : Action
    {
        private CharacterBase by;
        private float startTime;
        public GetShovedCV(CharacterBase by)
        {
            this.by = by;
        }
        ...
        public override ActionTransition OnResume(Actor actor)
        {
            ...
            {
                if(by.CanInteract(actor.GetCharacter()))
                {
                    return new ActionTransitionChangeTo(new UseInteractable(by), "How dare you!");
                }
